...
author: "Javier Herrero Pérez, Jose Aguilar Milla"
date: "2025-12-05"
output:
...
Se busca estudiar crim de manera lineal con la variable que tenga mayor relación con esta, para ello se va a estudiar la matriz de correlación eliminando la variable crim:
...
plot(df$lstat, df$crim,
     ...
     pch = 19, col = "blue")
```
la relación es positiva y moderada. Gráficamente, se observa una tendencia creciente, aunque con mucha dispersión, especialmente para valores altos de crim.
...
legend("topright",
       ...
       bty = "n")
```
Se observa que la banda de confianza que estima el rango para la media de crim dado un valor de lstat, es relativamente estrecha. Sin embargo, una gran cantidad de valores individuales están excluidos de esta banda. Esto es una consecuencia directa de una varianza residual alta.
...
El objetivo es encontrar el modelo que logre el mejor equilibrio entre buena capacidad predictiva y simplicidad. Para ello el objetivo es ver qué modelo simple maximiza $R^2$ minimizando $C_p$ y BIC. El mínimo de $C_p$ nos lleva a que el modelo tiene la menor suma de errores debidos a la varianza y al sesgo. Por otro lado un BIC bajo indica que el modelo alcanza una buena verosimilitud con pocos parámetros (principio de parsimonia).

Las gráficas de RegSubsets para log(crim) muestran que el $R^2$ se maximiza con 8 variables. Sin embargo, el mínimo de $C_p$ y BIC se encuentran para 7 variables, es por eso que se decide optar por un modelo de 7 variables.
...
El método de stepwise selecciona el modelo óptimo de manera autmática dado un conjunto de variables predictoras.

```{r}
df <- read_excel("boston.xlsx")
lm3<-lm(log(crim)~.,data=df)
lm3_stepwise<-step(lm3)
summary(lm3_stepwise)
```

El modelo obtenido con stepwise tiene un $R^2$ ligeramente mejor y un AIC más bajo. SIn embargo, el modelo RegSubsets considero que es mejor porque simplifica más la complejidad. Si la diferencia de $R^2$ es mínima, se prefiere el modelo más simple, en este caso RegSubsets.


El modelo de stepwise explica un $83,93\%$ de la varianza. 

En cuanto el efecto de la variables chas sobre crim, se observa que el coeficiente de chas es negativo y estadísticamente significativo.

## Apartado 3

En el apartado anterior se ha podido observar cómo hay variables que no son estadísticamente significativas. En este apartado se va a realizar el diagnóstico de ese modelo sin emprender ninguna acción de eliminar esas variables.

El problema principal de este modelo es que se incluyen variables no significativas según los resultados vistos con la función `summary()`. Esto sugiere que el modelo ha tenido en cuenta las variables significativas y las que no lo son (age, medv) no es estadísticamente relevante. Estova en contra del principio de parsimonia.

```{r}
plot(lm3_stepwise)
```

La gráfica de Residuals vs Fitted, la línea roja de la media de los residuos está prácticamente plana en torno a cero. Se ha mejorado la homocedasticidad de los residuos. 
En cuanto al QQ-residuals se observa que presenta una tendencia esperada, aunque se observa una ligera desviación del supuesto de normalidad de los residuos, sobretodo para valores extremos.

Por último, en la figura de Residuals vs Leverage se identifican valores outiliers con altos residuos que pueden estar afectando la robustez del modelo.

## Apartado 4

Uno de los principales problemas que se tiene con stepwise es que utiliza age y medv. El problema de la heterocedasticidad en los residuos ya estaba resuelto por haber usado el log(crim), si no se hubiera puesto desde un principio hubiera sido otra mejora a considerar

```{r}
df <- read_excel("boston.xlsx")

lm3<-lm(log(crim)~.-age-medv,data=df)
lm3_stepwise<-step(lm3)

summary(lm3_stepwise)
```

Se observa que ahora el valor de AIC sale ligeramente más alto que antes tras haber eliminado estos valores, y también el coeficiente de $R^2$, sin embargo, ahora todas las variables son estadísticamente significativas. Tras eliminar estas variables se obtiene un modelo más simple y con coeficientes más robustos.

```{r}
plot(lm3_stepwise)
```
Tras el reajuste, los problemas de no linealidad y heterocedasticidad están resueltos (gracias a que se utiliza log(crim)) y eliminar las variables más débiles ha ayudado a que el modelo sea más preciso para interpretar.


## Apartado 5

```{r}
medv_indus <- median(df$indus)
medv_nox <- median(df$nox)
medv_rm <- median(df$rm)
medv_ptratio <- median(df$ptratio)
medv_lstat <- median(df$lstat)
medv_dis<-median(df$dis)

new_data_median <- data.frame(indus = medv_indus,chas = 0,nox = medv_nox,rm = medv_rm,dis=medv_dis,ptratio =medv_ptratio,lstat = medv_lstat)

IP_log_crim <- predict(lm3_stepwise, 
...
print(IP_crim)
print(median(df$crim))
```
Para la variable categórica chas se ha decidido utilizar 0 por conveniencia.

El resultado obtenido es lógico, dentro del intervalo de confianza se encuentra el valor de la mediana por lo que se puede concluir que el modelo es robusto capaz de explicar aproximadamente un $84\%$ de la varianza de los datos que permite predecir la tasa de criminalidad esperada para un determinado barrio.
